...
Matrix createLocalK(int e,mesh &m){
    float VARtao,VARkappa,VARlambda,VARdelta,VAReta,Ae,J,D;
    Matrix matrixA,matrixB,matrixC,matrixD, matrixE, matrixAe;
    ...
    VARtao = m.getParameter(TAO);
    productRealMatrix(VARtao*J/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BVAReta,2,2,6),6,2,6),matrixA);
    VARkappa = m.getParameter(KAPPA);
    productRealMatrix(VARkappa*Ae/(D*D),productMatrixMatrix(BVARetat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BVAReta,2,2,6),2,2,6),6,2,6),matrixB);
    VARdelta = m.getParameter(DELTA);
    productRealMatrix((-1)*(VARdelta*Ae)/(8*D*D),productMatrixMatrix(BVARetat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixAe);
    VARlambda = m.getParameter(LAMBDA);
    productRealMatrix((J*VARlambda)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixE);
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(PSIX);
    float f_y = m.getParameter(PSIY);
    float VAReta = m.getParameter(ETA);
